<%@ page import="org.springframework.security.core.context.SecurityContextHolder; java.text.SimpleDateFormat" %>
<div id="calendar">
    ...
    %>
    <div class="list">
        <a href="#" onclick="moveCalendar(${cal.get(Calendar.MONTH)}, ${cal.get(Calendar.YEAR)})"><g:formatDate
                date="${cal.time}" format="MMMM"/></a>
        &nbsp;|&nbsp;
        <%
        ...
        <table class="cal">
            ...
            <g:each in="${(fromCal.time)..(toCal.time)}" var="date" status="idx">
                ...
                <g:render template="day"
                          model="[actualMonth: fromDate.month, date: date, efforts: effortsByDate[(date)], editable: editable]"/>
                <g:if test="${idx % 7 == 6}">
                    </tr>
                </g:if>
            </g:each>
