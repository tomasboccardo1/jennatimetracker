...
class SalutationStep3RequestHandler extends RequestHandler {
    ...
    public static Answer answerEvaluation(String msg) {
    ...
    public static List<Response> getAnswerFor(Answer answer) {
    ...
    def doHandle(Conversation _conversation, ChatService _chatService) {
        ...
        // Ask about humor
        if (answer == Answer.NEGATIVE) {
            _conversation.responses << Response.build('negativeHumourHandler')
            _conversation.context.askForHumour = true
            _conversation.context.moodValue= Integer.parseInt(actualMessage);
            return
        }else{
